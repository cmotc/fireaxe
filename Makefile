...
config:
	lb config --image-name awesome \
		--linux-flavours "linux-image-grsec,linux-image"
...
config-frsec:
	lb config --image-name awesome-frsec \
		--linux-flavours "linux-image-grsec"
...
usb:
	sudo dd if=awesome-amd64.hybrid.iso of=/dev/sdc
